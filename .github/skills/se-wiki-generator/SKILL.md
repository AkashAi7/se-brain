...
description: 'Incrementally build and maintain an LLM-generated wiki from raw source documents. Use when asked to "build the wiki", "generate wiki", "ingest sources", "update wiki", "compile wiki", "create wiki pages", "synthesize sources", "link concepts", "lint wiki", or "health-check wiki". Reads raw sources from Azure DevOps raw/ plus the optional local KB-Local/ folder, and produces a structured, interlinked collection of markdown files in the LOCAL wiki/ — summaries, entity pages, concept pages, comparisons, an overview, and a synthesis. Source references point to the Azure DevOps project. Maintains cross-references, backlinks, index, and changelog automatically.'
---
...
The shared raw dump now lives in Azure DevOps, with an optional private local layer. The wiki you build is the **local** synthesized layer on top of it.

| Layer | Where it lives | Role | How to access |
|-------|----------------|------|---------------|
| **Raw dump — shared** | Azure DevOps project `SE-Brain-AzDev/SE-Brain` under repo path `raw/` | Authoritative source documents | Azure DevOps MCP tools or a confirmed-current local checkout |
...
**Direction of work:** read raw from Azure DevOps `raw/` **+ KB-Local** -> write the compiled wiki **locally** in `wiki/`.

- **NEVER** publish generated `wiki/` pages into the raw source store unless the user explicitly asks — the wiki is a local artifact. Read and write it with the local file tools.
- **NEVER** read a stale local `raw/` mirror as authoritative. Use Azure DevOps MCP tools, or read local `raw/` only when the workspace is a confirmed-current checkout. `KB-Local/` is the only sanctioned private local *source* read.
- **Source references in wiki pages point to Azure DevOps.** Convert any raw path to a clickable Azure DevOps URL:
   `https://dev.azure.com/SE-Brain-AzDev/SE-Brain/_git/SE-Brain?path=/<path>` (spaces as `%20`). KB-Local-derived material is labeled "From local notes (KB-Local)" and has no Azure DevOps URL.
...
- Azure DevOps MCP access, or a confirmed-current local checkout, connected to the Azure DevOps raw dump
- At least one source file in Azure DevOps `raw/` (or in local `KB-Local/`)
- Each shared source in Azure DevOps should have YAML frontmatter with `title`, `url`, `date_retrieved`, `source_type`, and `tags`
- `raw/sources.md` manifest in Azure DevOps (recommended but not required — the skill can enumerate `raw/` through Azure DevOps MCP or the local checkout)

## Directory Structure

The skill creates and maintains the following structure **locally** (Azure DevOps only holds the shared `raw/` sources):
...
> **`sources:` paths are Azure DevOps raw paths.** Entries like `raw/source-slug.md` identify documents in the Azure DevOps raw dump. When you render them as clickable links in page bodies, expand them to the Azure DevOps URL: `https://dev.azure.com/SE-Brain-AzDev/SE-Brain/_git/SE-Brain?path=/raw/source-slug.md`. A source that came from `KB-Local/` instead gets a `kb-local/<file>.md` entry and is attributed inline as "From local notes (KB-Local)" — it has no Azure DevOps URL. `backlinks:` paths stay as local `wiki/` relative paths.
...
> Source: [Original Title](original-url) | On Azure DevOps: [raw/<source-slug>.md](https://dev.azure.com/SE-Brain-AzDev/SE-Brain/_git/SE-Brain?path=/raw/<source-slug>.md) | Retrieved: YYYY-MM-DD | Type: article
...
Triggered when new source(s) land in Azure DevOps `raw/` (or in local `KB-Local/`) and the user asks the LLM to process them. This is the most common operation.

**Workflow:**

1. **Identify new sources.**
   - Shared: read `raw/sources.md` or enumerate `raw/` through Azure DevOps MCP tools or a confirmed-current local checkout. Compare against the local `wiki/index.md` to find sources not yet ingested.
...
2. **Read each new source.**
   - Shared sources: read `raw/<file>.md` from Azure DevOps — parse the frontmatter and full content directly from the fetched content. Do NOT write fetched tool output to a temp file and re-parse it.
...
For bootstrapping — when there's no local wiki yet but Azure DevOps `raw/` (and/or `KB-Local/`) has multiple sources. Runs Ingest on every source, then does a final cross-referencing pass.
...
3. Enumerate every source through Azure DevOps MCP tools or a confirmed-current local checkout (shared) and the `KB-Local/` scan (private), then ingest each one by one (follow Ingest workflow above).
...
3. Fetch the *specific* raw documents referenced by those wiki pages from Azure DevOps `raw/` (or `read_file` on the `KB-Local/` note). Don't blanket-scan the raw dump.
4. Synthesize an answer with citations to local wiki pages and ultimately to the Azure DevOps raw URLs.
...
| Issue | Solution |
|-------|---------|
| Azure DevOps `raw/` empty and no `KB-Local/` notes | No sources to ingest. Run the **se-open-research** / **se-work-research** skill to populate the Azure DevOps raw dump first. |
| Azure DevOps MCP unavailable or auth error | Tell the user Azure DevOps MCP interactive auth or repo permissions are required. Do NOT fall back to a stale local `raw/` mirror. |
