...
- Azure DevOps MCP access, or a confirmed-current local checkout, for fetching specific raw documents from Azure DevOps when the wiki points to them
...
1. **Ground every claim in a source.** Cite local wiki pages with relative links and raw sources with Azure DevOps URLs:
   - `(see [Transformer Architecture](../concepts/transformer-architecture.md))` — local wiki page
   - `(from [Attention Is All You Need](https://dev.azure.com/SE-Brain-AzDev/SE-Brain/_git/SE-Brain?path=/raw/attention-is-all-you-need.md))` — Azure DevOps raw source
   - For private notes, attribute inline as "From your local notes (KB-Local)" — they are local-only and have no Azure DevOps URL.

2. **Present contradictions honestly.** If sources disagree:
   > Source A claims X (from [Source A](https://dev.azure.com/SE-Brain-AzDev/SE-Brain/_git/SE-Brain?path=/raw/source-a.md)), while Source B claims Y (from [Source B](https://dev.azure.com/SE-Brain-AzDev/SE-Brain/_git/SE-Brain?path=/raw/source-b.md)). The wiki has flagged this contradiction — see [Concept Page](../concepts/concept.md#contradictions).
...
1. Save locally to `wiki/analyses/<slug>.md` with proper frontmatter (`sources:` lists the Azure DevOps raw paths, e.g. `raw/source1.md`, that the analysis draws on):
...
- **Always link your sources.** Every answer ends grounded: Azure DevOps URLs for the source docs you fetched, page titles/relative links for the wiki pages you used. No sourced answer ships without references.
...
| Issue | Solution |
|-------|---------|
| `wiki/index.md` missing | The local wiki hasn't been built. Run **se-wiki-generator** first (it reads the Azure DevOps raw dump + KB-Local). |
| Index is stale | Run **se-lint-wiki** to check index consistency, then rebuild. |
| Azure DevOps raw doc read fails / auth error | Tell the user Azure DevOps MCP interactive auth or repo permissions are required. Answer from the wiki alone and flag the missing detail — do not fall back to a stale local `raw/` mirror. |
