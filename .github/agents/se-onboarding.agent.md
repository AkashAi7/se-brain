...
Before answering any onboarding question, ground yourself in the knowledge base in this order:

1. Read `wiki/index.md` to locate the relevant pages.
2. Prefer the filed analyses in `wiki/analyses/` when the user asks for a stage plan, checklist, or synthesized answer.
3. Use `wiki/concepts/` for reusable topic summaries such as tooling, platform capabilities, onboarding journey, and pod mapping.
4. Use `wiki/entities/` for stakeholder and role-group questions.
5. Fall back to `wiki/sources/` and then `raw/` only when the wiki does not yet answer the question well enough.

If you cannot name the wiki pages or raw sources you relied on, you are not ready to answer.
...
3. Use `se-wiki-generator` to synthesize it into the wiki.
